...
public class MainWindow extends JFrame
{
    ...
    public MainWindow(Controller inController)
    {
        ...
        writeReminders(controller.getReminders());
    ...
    /**
     * Add observers to ReminderList through Controller
     */
    private void setupObservers()
    {
        ReminderObserver obViewer;
        ReminderObserver obWriter;

        obViewer = new ReminderViewer(this); //Observer for displaying 
        obWriter = new ReminderWriter(this); //Observer for writing to file

        controller.addObserver(obViewer);
        controller.addObserver(obWriter);
    }
    ...
    /**
     * Write updated reminder list to output file.
     * @param reminders
     */
    public void writeReminders(List<Reminder> reminders)
    {
        try
        {
            FileManager.write(REMINDER_FILE, reminders);
        }
        catch(IOException e)
        {
            System.err.println("Warning: unable to open the reminders file!" +
                                " Cannot write reminders to file. Continuing without it.");
        }
    }
}
